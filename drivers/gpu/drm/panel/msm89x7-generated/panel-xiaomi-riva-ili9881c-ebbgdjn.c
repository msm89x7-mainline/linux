...
static const struct regulator_bulk_data ili9881c_ebbgdjn_c3b_supplies[] = {
	{ .supply = "vsn" },
	{ .supply = "vsp" },
};
...
static int ili9881c_ebbgdjn_c3b_prepare(struct drm_panel *panel)
{
	...
	ret = regulator_bulk_enable(ARRAY_SIZE(ili9881c_ebbgdjn_c3b_supplies), ctx->supplies);
	if (ret < 0) {
	...
	if (ret < 0) {
		...
		gpiod_set_value_cansleep(ctx->reset_gpio, 1);
		regulator_bulk_disable(ARRAY_SIZE(ili9881c_ebbgdjn_c3b_supplies), ctx->supplies);
		return ret;
	}
...
static int ili9881c_ebbgdjn_c3b_probe(struct mipi_dsi_device *dsi)
{
	...
	ret = devm_regulator_bulk_get_const(dev,
					    ARRAY_SIZE(ili9881c_ebbgdjn_c3b_supplies),
					    ili9881c_ebbgdjn_c3b_supplies,
					    &ctx->supplies);
	if (ret < 0)
		return ret;
...
static const struct of_device_id ili9881c_ebbgdjn_c3b_of_match[] = {
	{ .compatible = "xiaomi,riva-ili9881c-ebbgdjn" }, // FIXME
	{ /* sentinel */ }
};
...
static struct mipi_dsi_driver ili9881c_ebbgdjn_c3b_driver = {
	...
	.driver = {
		.name = "panel-xiaomi-riva-ili9881c-ebbgdjn",
